...
render::load_shader(gl::GLenum type, const std::string& code) -> Shader
{
  ...
  utils::throw_on_opengl_error("Failed to load shader's resource");

  gl::glCompileShader(shader);
  utils::throw_on_opengl_error("Failed to compile shader");

  gl::GLboolean is_compiled_flag;
  gl::glGetShaderiv(shader, gl::GL_COMPILE_STATUS, &is_compiled_flag);
  utils::throw_on_opengl_error("Failed to get compile status");
  ...
  utils::throw_on_opengl_error("Failed to get shader log");
...
render::link_program(const std::vector<Shader>& shaders) -> Program
{
  ...
  for (auto& shader : shaders) {
    ...
    utils::throw_on_opengl_error("Failed to attach shader");
  }
  ...
  for (auto& shader : shaders) {
    ...
    utils::throw_on_opengl_error("Failed to deattach shader");
  }

  gl::GLboolean is_linked_flag;
  gl::glGetProgramiv(program, gl::GL_LINK_STATUS, &is_linked_flag);
  utils::throw_on_opengl_error("Failed to get link status");
  ...
  utils::throw_on_opengl_error("Failed to get program log");
